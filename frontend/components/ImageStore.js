...
const StarIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="none" className="text-yellow-400">
      ...
      <div className="relative w-full max-w-5xl h-[80vh] bg-[#0d1117]/90 border border-[#30363d] rounded-xl flex flex-col shadow-2xl overflow-hidden ring-1 ring-white/10">
        <div className="flex items-center justify-between p-6 border-b border-[#30363d] bg-[#161b22]/50">
          <div>
            <h2 className="text-xl font-bold text-white tracking-wide flex items-center gap-2">
              DOCKER HUB{" "}
              <span className="text-[#1f6feb] text-xs px-2 py-0.5 rounded border border-[#1f6feb]/30 bg-[#1f6feb]/10">
                REGISTRY
              </span>
            </h2>
            <p className="text-xs text-[#8b949e] mt-1">
              Select an image to pull into your local environment.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 scrollbar-thin scrollbar-thumb-gray-700">
          {data &&
            data.map((img, idx) => {
              ...
              const desc =
                img.Description || img.description || "No description provided.";
              const stars = img.StarCount || img.star_count || 0;
              const isOfficial =
                img.IsOfficial || img.is_official || false;
              const isAutomated =
                img.IsAutomated || img.is_automated || false;
              ...
              return (
                ...
                >
                  <div>
                    <div className="flex items-start justify-between mb-2">
                      <h3
                        className="text-lg font-bold text-[#c9d1d9] group-hover:text-[#58a6ff] truncate pr-2"
                        title={name}
                      >
                        {name}
                      </h3>
                    ...
                    <p
                      className="text-sm text-[#8b949e] mb-4 line-clamp-2 h-10"
                      title={desc}
                    >
                      {desc}
                    </p>
